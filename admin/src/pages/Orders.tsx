...
import { ShoppingCart, Eye, Package, MessageCircle, Send, User, UserCheck, History } from 'lucide-react';
...
export default function Orders() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`messages-${orderDetails.telegram_id}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
              ...
              {orderDetails && (
                <Card>
                  <CardContent className="p-6">
                      ...
                      {messagesLoading ? (
                        ...
                        <>
                          {messages.map((msg: any, index: number) => {
                            // Check if this message is from a different order than the previous one
                              ...
                              <div key={msg.id}>
                                ...
                                >
                                  ...
                                  <div className={cn(
                                    ...
                                    {/* Show order context if different from current */}
                    ...
                    {/* Send message - only if order is assigned to me or I'm admin */}
                    {(isMyOrder(orderDetails) || user?.role === 'admin') ? (
                      <div className="flex gap-2">
                        <Input
                          value={newMessage}
                    ...
                    ) : orderDetails.assigned_manager_id ? (
                      <div className="text-center text-sm text-muted-foreground py-2 border rounded-lg">
                        Чат ведёт: {orderDetails.assigned_manager?.email}
                      </div>
                    ) : (
                      <div className="text-center text-sm text-muted-foreground py-2 border rounded-lg">
                        Возьмите заказ, чтобы писать клиенту
                      </div>
                    )}
                  </CardContent>
                </Card>
